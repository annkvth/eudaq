...
int main(int /*argc*/, const char **argv) {
  ...
  std::map<uint32_t, uint32_t> lastTrigId;
  ...
  while(1){
    ...
    if((in_range_evn && in_range_tgn && in_range_tsn) && not_all_zero){
      ev->PrintTable(std::cout);
// check Trigger
    auto subEvs = ev->GetSubEvents();
    for(auto &subev: subEvs) {
	auto iTrig = lastTrigId.find(subev->GetStreamN());
	  if ( subev->GetDescription() != "NiRawDataEvent" && iTrig != lastTrigId.end() && iTrig->second != subev->GetTriggerN() - 1)
		std::cout << "Trigger inconsistency for " << subev->GetDescription() << std::endl;
	  lastTrigId[subev->GetStreamN()] = subev->GetTriggerN();
    }
}
